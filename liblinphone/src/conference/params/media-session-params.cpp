...
void MediaSessionParamsPrivate::clone (const MediaSessionParamsPrivate *src) {
	...
	m_cameraParam = src->m_cameraParam;
}
...
const std::string& MediaSessionParams::getCameraParam()
{
	L_D();
	return d->m_cameraParam;
}
void MediaSessionParams::setCameraParam(const std::string &param) {
	L_D();
	d->m_cameraParam = param;

	lInfo() << "******CUSTOM****** MediaSessionParams::setCameraParam < " << d->m_cameraParam << " > ------";
}
