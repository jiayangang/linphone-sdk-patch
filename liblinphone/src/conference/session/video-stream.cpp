...
void MS2VideoStream::render(const OfferAnswerContext & ctx, CallSession::State targetState){
	...
	if (getCCore()->video_conf.reuse_preview_source && source) {
		...
		if (ok) {
			...
			std::string cam_addr = getMediaSessionPrivate().getParams()->getCameraParam();
			lWarning() << "******CUSTOM****** IPC Camera Param ------< " << cam_addr << " >-----";
			video_stream_set_cam_param(mStream, cam_addr.c_str());
