...
/**
 * Get the native window handle of the preview window, casted as an unsigned long.
**/
LINPHONE_PUBLIC void * linphone_call_get_native_preview_window_id(const LinphoneCall *call);

/**
 * Set the native preview window id where the video is to be displayed.
 * For MacOS, Linux, Windows: if not set or 0 a window will be automatically created, unless the special id -1 is given.
**/
LINPHONE_PUBLIC void linphone_call_set_native_preview_window_id(LinphoneCall *call, void *id);
