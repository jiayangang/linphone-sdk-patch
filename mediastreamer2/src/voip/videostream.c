...
void video_stream_free(VideoStream *stream) {
	...
	if(stream->cam_param)
		ms_free(stream->cam_param);

	if(stream->prev_decoder)
		ms_filter_destroy(stream->prev_decoder);
	if (stream->h26xpack)
		ms_filter_destroy(stream->h26xpack);
...
void video_stream_set_cam_param(VideoStream *vs, const char *param){
	if (vs->cam_param!=NULL){
		ms_free(vs->cam_param);
		vs->cam_param=NULL;
	}
	if (param!=NULL && *param != '\0')
		vs->cam_param=ms_strdup(param);

	ms_warning("***CUSTOM***  video_stream_set_cam_param %s\n", param);
}
	...
	if (media_stream_get_direction(&stream->ms) != MediaStreamRecvOnly){
		switch(io->input.type){
			...
			case MSResourceCamera:
				...
				if(source) ms_filter_call_method(source, MS_FILTER_SET_CAM_PARAM, stream->cam_param);
			break;
			case MSResourceFile:
...
extern MSFilterDesc ms_h26x_pack_desc;
static int video_stream_start_with_source_and_output(VideoStream *stream, RtpProfile *profile, const char *rem_rtp_ip, int rem_rtp_port,
	const char *rem_rtcp_ip, int rem_rtcp_port, int payload, int jitt_comp, MSWebCam *cam, MSFilter *source, MSFilter *output) {
	PayloadType *pt;
    ...
    if (source)
	{
		int get_source_fmt = MS_SOURCE_FMT_DEFAULT;
		ms_filter_call_method(source, MS_FILTER_GET_SOURCE_FMT, &get_source_fmt);
		if(MS_SOURCE_FMT_ENCODED == get_source_fmt) {
			
			stream->source_performs_encoding = TRUE;   // 	source data is encoded
		}
	}
	...
	if (media_stream_get_direction(&stream->ms) == MediaStreamRecvOnly) {
		...
		if (rtp_source) {
			...
			if (stream->source_performs_encoding == TRUE) {
				...
				stream->h26xpack = ms_factory_create_filter_from_desc(stream->ms.factory, &ms_h26x_pack_desc);//ms_factory_create_filter(stream->ms.factory, MS_H26X_PACK_ID);
				//stream->prev_decoder = ms_factory_create_filter_from_name(stream->ms.factory, "MSH264DecForPrev");//// "" add 2020-11-10
				stream->prev_decoder = ms_factory_create_decoder(stream->ms.factory, pt->mime_type);

				//MSPixFmt prev_fmt = MS_H264;
				//ms_filter_call_method(stream->prev_decoder, MS_FILTER_SET_PIX_FMT, &prev_fmt);

				/* set parameters to the decoder*/
				//if (pt->send_fmtp){
					//ms_filter_call_method(stream->prev_decoder,MS_FILTER_ADD_FMTP,pt->send_fmtp);
				//}
				//if (pt->recv_fmtp!=NULL)
					//ms_filter_call_method(stream->prev_decoder,MS_FILTER_ADD_FMTP,(void*)pt->recv_fmtp);
			}
		...
		if (stream->source_performs_encoding == TRUE && stream->h26xpack) {
			ms_connection_helper_link(&ch, stream->h26xpack, 0, 0);
		}
		...
		if (stream->output2){
			...
			if (stream->source_performs_encoding == TRUE)
			{
				ms_filter_link(stream->tee, 1, stream->prev_decoder, 0);
				ms_filter_link(stream->prev_decoder, 0, stream->output2,0);
			}
			else {
	...
	if (stream->ms.sessions.ticker && stream->source){
		...
		if (change_source) {
			if (sink){
				...
				if(stream->source) {
					ms_filter_call_method(stream->source, MS_FILTER_SET_CAM_PARAM, stream->cam_param);

					int get_source_fmt = MS_SOURCE_FMT_DEFAULT;
					ms_filter_call_method(stream->source, MS_FILTER_GET_SOURCE_FMT, &get_source_fmt);
					if(MS_SOURCE_FMT_ENCODED == get_source_fmt) {
						stream->source_performs_encoding = TRUE;   // source data is encoded
					}
				}
		...
		if (stream->source_performs_encoding == TRUE) {
			ms_filter_unlink(stream->ms.encoder, 0, stream->ms.rtpsend, 0);  // TODO: 2020-11-9
			stream->ms.encoder = stream->source;	/* Consider the encoder is the source */
			if(!stream->h26xpack) stream->h26xpack = ms_factory_create_filter_from_desc(stream->ms.factory, &ms_h26x_pack_desc); // TODO: 2020-11-12
			//stream->prev_decoder = ms_factory_create_filter_from_name(stream->ms.factory, "MSH264DecForPrev");
			{
				PayloadType *pt;
				RtpProfile *profile;
				int payload;
		...
		if (encoder_has_builtin_converter || (stream->source_performs_encoding == TRUE)) {
			...
			ms_filter_link (stream->tee, 0, stream->h26xpack, 0);        // TODO:2020-11-12
			ms_filter_link (stream->h26xpack, 0, stream->ms.rtpsend, 0); // TODO:2020-11-12
			...
			*/
			...
			ms_filter_link (stream->source, 0, stream->h26xpack, 0);
			ms_filter_link (stream->h26xpack, 0, stream->tee, 0);
	...
	if (stream->ms.sessions.ticker){
		if (stream->ms.state == MSStreamPreparing) {
			...
			if (stream->source){
				...
				if (stream->source_performs_encoding == TRUE && stream->h26xpack) {
					ms_connection_helper_unlink(&ch, stream->h26xpack, 0, 0);
				}
				if (stream->tee) {
				...
				if (stream->output2){
					if (stream->source_performs_encoding == TRUE)
					{
						ms_filter_unlink(stream->tee, 1, stream->prev_decoder, 0);
						ms_filter_unlink(stream->prev_decoder, 0, stream->output2,0);
					}
					else {
						ms_filter_unlink(stream->tee,1,stream->output2,0);
					}
...
#endif
	if (stream->source){
		//TODO ms_filter_call_method(stream->source,MS_VIDEO_DISPLAY_SET_NATIVE_WINDOW_ID,&id);
	}
}
