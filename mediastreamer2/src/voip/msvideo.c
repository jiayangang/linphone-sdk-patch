...
mblk_t* ms_buf_copy_from_ms_frame(YuvBuf *buf, int w, int h)
{
	YuvBuf dst_yuv;
	mblk_t *msg = ms_yuv_buf_alloc(&dst_yuv, w, h);

	MSVideoSize roi;
	roi.width = w;
	roi.height = h;
	ms_yuv_buf_copy(buf->planes, buf->strides, dst_yuv.planes, dst_yuv.strides, roi);
	return msg;
}
mblk_t* ms_buf_copy_from_buf(uint8_t *ptr, int size, int w, int h)
{
