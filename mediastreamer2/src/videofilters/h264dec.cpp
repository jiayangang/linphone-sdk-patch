...
typedef struct _DecPrevData{

	// ----------
	AVCodecContext *m_av_ctx;
	AVFrame	 *m_pFrame;  
...
int H264DecodeVideo_ToParse(MSFilter *f, DecPrevData *d, unsigned char *buffer, int data_len)
{
    int result = 0;
    int in_len = data_len;
    ...
    while (in_len>0) 
    {
        int lens = av_parser_parse2(d->pParserCtx, d->m_av_ctx, &out_buffer, &out_len, in_buffer, in_len, 0, 0, 0);
        in_len -= lens;
        in_buffer += lens;
        ...
        //print_pict_type(pParserCtx->pict_type, tem_count);
        d->package.data = out_buffer;
        d->package.size = out_len;
        int got_picture = -1;
        int ret = avcodec_decode_video2(d->m_av_ctx, d->m_pFrame, &got_picture, &d->package);
        if (ret < 0)
	    {
		    printf("Decode Error.\n");
		    return 0;
	    ...
	    {
			YuvBuf yuv420p;
			yuv420p.w = d->m_av_ctx->width;
			yuv420p.h = d->m_av_ctx->height;
			yuv420p.planes[0] = d->m_pFrame->data[0];
			yuv420p.planes[1] = d->m_pFrame->data[1];
			yuv420p.planes[2] = d->m_pFrame->data[2];
			yuv420p.strides[0] = d->m_pFrame->linesize[0];
			yuv420p.strides[1] = d->m_pFrame->linesize[1];
			yuv420p.strides[2] = d->m_pFrame->linesize[2];

			mblk_t *om = ms_buf_copy_from_ms_frame(&yuv420p, yuv420p.w, yuv420p.h);
			mblk_set_timestamp_info(om, d->cur_ts);
            //m_listener->cbDecodedFrame(m_pFrame, m_av_ctx->width, m_av_ctx->height);
		    //result = picture;
...
void  H264DecodeVideo_CloseDecoder(DecPrevData *d)
{
	if(d->m_pFrame) 
	{
		av_frame_free(&d->m_pFrame);
		d->m_pFrame = NULL;
	}
}
bool  H264DecodeVideo_OpenDecoder(DecPrevData *d)
{
    ...
    d->m_pFrame = av_frame_alloc();
    if (!d->m_pFrame) {
        perror("alloc frame err");
        return  false;
    }
    d->m_av_ctx = avcodec_alloc_context3(pCodec);
    d->m_av_ctx->codec_id = codec_id;
    if (!d->m_av_ctx) {
        perror("pcodecx err");
        return  false;
    }

    d->pParserCtx = av_parser_init(codec_id);
    if (!d->pParserCtx) {
    ...
    av_init_packet(&d->package);
    int open_ret = avcodec_open2(d->m_av_ctx, pCodec, NULL);
    if (open_ret<0) {
...
static void dec_prev_process(MSFilter *f){
	DecPrevData *d=(DecPrevData*)f->data;
	mblk_t *im;
	while((im=ms_queue_get(f->inputs[0]))!=NULL){

		d->cur_ts = mblk_get_timestamp_info(im);

		H264DecodeVideo_ToParse(f, d, im->b_rptr, (im->b_wptr - im->b_rptr));
		freemsg(im);
	}
}

static void dec_prev_postprocess(MSFilter *f) {
	//DecPrevData *s=(DecPrevData*)f->data;
	
}
static void dec_prev_preprocess(MSFilter* f) {
	DecPrevData *s = (DecPrevData*)f->data;
	H264DecodeVideo_OpenDecoder(s);
}

static void dec_prev_uninit(MSFilter *f) {
	DecPrevData *d = (DecPrevData*)f->data;
	ms_free(d);
}

static void dec_prev_init(MSFilter *f) {
	DecPrevData *d = ms_new0(DecPrevData, 1);
	f->data = d;
}

static MSFilterMethod  h264_dec_prev_methods[] = {
	{ 0                                                  ,	NULL }
};
...
MSFilterDesc ms_h264dec_prev_desc={
	.id=MS_H264_DEC_ID_2,
	.name="MSH264DecForPrev",
	.text="A H264 decoder based on ffmpeg project.",
	.category=MS_FILTER_DECODER,
	.enc_fmt="H264-2",
	.ninputs=1,
	.noutputs=1,
	.init=dec_prev_init,
	.preprocess=dec_prev_preprocess,
	.process= dec_prev_process,
	.postprocess=dec_prev_postprocess,
	.uninit=dec_prev_uninit,
...
void __register_ffmpeg_h264_decoder_if_possible(MSFactory *obj) {
	...
	if (avcodec_find_decoder(CODEC_ID_H264))// && HAVE_NON_FREE_CODECS) 
	{
		...
		// ms_factory_register_filter(obj, &ms_h264dec_prev_desc);
	}
}
