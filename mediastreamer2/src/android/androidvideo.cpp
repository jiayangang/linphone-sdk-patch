...
void video_capture_preprocess(MSFilter *f){
	...
	if (d->previewWindow) {
        ...
        method = env->GetStaticMethodID(d->helperClass,"setPreviewDisplaySurface", "(Ljava/lang/Object;Ljava/lang/Object;)V");
		env->CallStaticVoidMethod(d->helperClass, method, d->androidCamera, NULL);
	}
...
static void video_capture_process(MSFilter *f){
	...
	//ms_message("[Legacy Capture] video_capture_process 11111");
	ms_mutex_lock(&d->mutex);
	...
	//ms_message("[Legacy Capture] video_capture_process 22222");
}
